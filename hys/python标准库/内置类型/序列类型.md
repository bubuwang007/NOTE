...
由 [`dict.keys()`](https://www.bookstack.cn/read/python-3.10.0-zh/7adaf5f79c9e977a.md#dict.keys), [`dict.values()`](https://www.bookstack.cn/read/python-3.10.0-zh/7adaf5f79c9e977a.md#dict.values) 和 [`dict.items()`](https://www.bookstack.cn/read/python-3.10.0-zh/7adaf5f79c9e977a.md#dict.items) 所返回的对象是 *视图对象*。 该对象提供字典条目的一个动态视图，这意味着当字典改变时，视图也会相应改变。
...
键和值是按插入时的顺序进行迭代的。 这样就允许使用 [`zip()`](https://www.bookstack.cn/read/python-3.10.0-zh/343ac5efc1fa2896.md#zip) 来创建 `(值, 键)` 对: `pairs = zip(d.values(), d.keys())`。 另一个创建相同列表的方式是 `pairs = [(v, k) for (k, v) in d.items()]`.

在添加或删除字典中的条目期间对视图进行迭代可能引发 [`RuntimeError`](https://www.bookstack.cn/read/python-3.10.0-zh/177bd954d1cc6696.md#RuntimeError) 或者无法完全迭代所有条目。
